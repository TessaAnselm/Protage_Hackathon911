...
| RocketRide | **Live** | `backend/adapters/rocketride_adapter.py` — key authenticates against RocketRide's **staging** endpoint (`https://staging.rocketride.ai/`), not the default `https://cloud.rocketride.ai/`. Actually orchestrating: `/api/ask` ("Ask about this migration") builds a real RocketRide pipeline (`chat` source → `llm_openai` → `response`, per their documented `.pipe` schema), starts it with `client.use()`, and answers via `client.chat()` — the LLM call is made *by the RocketRide-hosted node*, not by us directly |
